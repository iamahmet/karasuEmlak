...
export async function generateMetadata({
  ...
  const canonicalPath = locale === routing.defaultLocale ? `/satilik/${tip}` : `/${locale}/satilik/${tip}`;
  ...
  return {
    ...
    openGraph: {
      title: `Satılık ${typeLabel} İlanları | Karasu Emlak`,
      description: `Karasu'da satılık ${typeLabel.toLowerCase()} ilanları`,
      url: `${siteConfig.url}${canonicalPath}`,
    },
  };
}

export default async function SatilikTipPage({
  params,
  searchParams,
}: {
  params: Promise<{ locale: string; tip: string }>;
  ...
  const breadcrumbs = [
    ...
    { label: `Satılık ${typeLabel}`, href: `${basePath}/satilik/${tip}` },
  ];
        ...
        <div className="container mx-auto px-4 py-8 lg:py-12">
          <header className="mb-8">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
              Satılık {typeLabel} İlanları
            </h1>
            <p className="text-lg text-gray-600 max-w-3xl">
              Karasu ve çevresinde satılık {typeLabel.toLowerCase()} ilanları. Denize sıfır konumlarda en güncel fırsatları keşfedin.
            </p>
          </header>
