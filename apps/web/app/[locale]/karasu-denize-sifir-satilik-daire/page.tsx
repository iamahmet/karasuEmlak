...
export default async function KarasuDenizeSifirSatilikDairePage({
  ...
  try {
    relatedArticles = await getRelatedContent({
      ...
      location: 'Karasu',
      category: 'Rehber',
      tags: ['Karasu', 'Daire', 'Denize Sıfır', 'Yatırım'],
      limit: 6,
    });
  } catch (error) {
    console.error('[KarasuDenizeSifirSatilikDairePage] Error fetching related articles:', error);
    relatedArticles = [];
  }
          ...
          <section className="py-16 bg-gray-50 dark:bg-gray-900">
            <div className="container mx-auto px-4">
              <EnhancedRelatedArticles
                ...
                basePath={basePath}
                title="Karasu Denize Sıfır Daire ve Yatırım Hakkında Makaleler"
                limit={6}
              />
            </div>
          </section>
