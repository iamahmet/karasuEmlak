...
export async function generateMetadata({
  ...
  const canonicalPath = locale === routing.defaultLocale ? '/satilik-yazlik' : `/${locale}/satilik-yazlik`;
  
  const baseDescription = 'Türkiye\'de satılık yazlık ilanları. 1+1\'den 4+1\'e kadar seçenek. Güncel fiyatlar, mahalle rehberi ve yatırım analizi. Uzman emlak danışmanlığı ile hayalinizdeki yazlığı bulun.';
  const optimizedDescription = optimizeMetaDescription(baseDescription, {
    keywords: ['satılık yazlık', 'yazlık ilanları', 'emlak'],
    includeCTA: true,
  });

  return {
    title: 'Satılık Yazlık | En Güncel İlanlar ve Fiyatlar 2025 | Karasu Emlak',
    ...
    openGraph: {
      ...
      description: 'Türkiye\'de satılık yazlik ilanları. 1+1\'den 4+1\'e kadar geniş seçenek. Güncel fiyatlar ve mahalle rehberi.',
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: 'Satılık Yazlık - Emlak İlanları ve Fiyatları',
        },
      ],
    ...
    twitter: {
      ...
      title: 'Satılık Yazlık | En Güncel İlanlar',
      description: 'Türkiye\'de satılık yazlik ilanları. Geniş seçenek. Güncel fiyatlar ve mahalle rehberi.',
    },
    robots: {
      index: true,
      follow: true,
...
async function getSatilikYazlikFAQs() {
  ...
  if (allFAQs.length === 0) {
    allFAQs.push(
      ...
      {
        question: 'Yazlık alırken hangi belgeler gereklidir?',
        answer: 'Yazlık alırken tapu, yapı ruhsatı, iskan belgesi, vekaletname (varsa), kimlik belgeleri, gelir belgesi (kredi için), vergi levhası ve diğer yasal belgeler gereklidir. Tüm belgelerin güncel ve eksiksiz olması önemlidir.',
      },
      {
      ...
      {
        question: 'Yazlık alırken ekspertiz yaptırmak gerekli mi?',
...
export default async function SatilikYazlikPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const yazlikListings = allListings.filter(listing => listing.property_type === 'daire');

  // Group by room count
  const byRooms = {
    '1+1': yazlikListings.filter(l => l.features?.rooms === 1),
    '2+1': yazlikListings.filter(l => l.features?.rooms === 2),
    '3+1': yazlikListings.filter(l => l.features?.rooms === 3),
    '4+1': yazlikListings.filter(l => l.features?.rooms === 4),
  };

  // Calculate average price
  const prices = yazlikListings
  ...
  const faqsResult = await withTimeout(getSatilikYazlikFAQs(), 2000, []);
  const faqs = faqsResult || [];

  // Generate schemas
  const articleSchema = {
    ...generateArticleSchema({
      headline: 'Satılık Yazlık | En Güncel İlanlar ve Fiyatlar 2025',
      description: 'Türkiye\'de satılık yazlik ilanları. 1+1\'den 4+1\'e kadar geniş seçenek. Güncel fiyatlar, mahalle rehberi ve yatırım analizi.',
  ...
  const breadcrumbSchema = generateBreadcrumbSchema(
    [
      ...
      { name: 'Satılık Yazlık', url: `${siteConfig.url}${basePath}/satilik-yazlik` },
    ],
    `${siteConfig.url}${basePath}/satilik-yazlik`
  );
  ...
  const itemListSchema = yazlikListings.length > 0
    ? generateItemListSchema(yazlikListings.slice(0, 20), `${siteConfig.url}${basePath}`, {
        name: 'Satılık Yazlık İlanları',
        description: `Türkiye'de ${yazlikListings.length} adet satılık daire ilanı. Geniş seçenek.`,
      })
    : null;
  // Generate page content for AI checker
  const pageContentInfo = generatePageContentInfo('Satılık Yazlık', [
    { 
      id: 'genel-bakis', 
      title: 'Satılık Yazlık Arayanlar İçin Genel Bakış', 
      content: 'Türkiye\'de satılık yazlık ilanları ve seçenekleri hakkında kapsamlı bilgi. Denize yakın konumlarda, uygun fiyatlı ve lüks seçeneklerde yazlık evler bulunmaktadır. Hem yaz tatilleri hem de yatırım amaçlı seçenekler mevcuttur. İstanbul\'a yakınlık, turizm potansiyeli ve doğal güzellikler, yazlık yatırımları için cazip bölgeler oluşturmaktadır.' 
    },
    { 
      id: 'ozelliklerine-gore', 
      title: 'Özelliklerine Göre Satılık Yazlık Seçenekleri', 
      content: 'Denize sıfır, bahçeli, havuzlu, geniş balkonlu ve manzaralı yazlık seçenekleri mevcuttur. 1+1, 2+1, 3+1 ve 4+1 oda seçenekleri bulunmaktadır. Her yazlık, konum, metrekare, oda sayısı ve özelliklerine göre farklı fiyat aralıklarında sunulmaktadır.' 
    },
    { 
      id: 'fiyat-analizi', 
      title: 'Satılık Yazlık Fiyat Analizi', 
      content: 'Satılık yazlık fiyatları konum, metrekare, oda sayısı, denize yakınlık ve özelliklere göre değişmektedir. Ortalama fiyatlar 600.000 TL ile 1.800.000 TL arasında değişmektedir. Denize yakın konumlar daha yüksek fiyatlara sahiptir. Yatırım amaçlı yazlık alımında kira getirisi, turizm potansiyeli ve bölgenin gelişim projeleri dikkate alınmalıdır.' 
    },
    { 
      id: 'mahalle-rehberi', 
      title: 'Mahalle Rehberi', 
      content: 'Popüler yazlık bölgeleri ve mahalleleri hakkında detaylı bilgi. Denize yakın mahalleler, merkez mahalleler, gelişen bölgeler ve uygun fiyatlı bölgeler. Her mahallenin ulaşım, sosyal alanlar, plaj erişimi ve turizm potansiyeli açısından değerlendirmesi.' 
    },
    { 
      id: 'yatirim-tavsiyeleri', 
      title: 'Yatırım Tavsiyeleri', 
      content: 'Yazlık yatırımı yapmayı düşünenler için kira getirisi, turizm potansiyeli, bölgenin gelişim projeleri ve piyasa trendleri hakkında tavsiyeler. Yatırım amaçlı yazlık alımında dikkat edilmesi gerekenler, getiri analizi ve risk değerlendirmesi.' 
    },
  ]);
    ...
    <>
      ...
      <Breadcrumbs
        items={[
          ...
          { label: 'Satılık Yazlık', href: `${basePath}/satilik-yazlik` },
        ]}
      />
              ...
              <div className="max-w-4xl mx-auto text-center">
                <div className="inline-block mb-4">
                  <span className="px-4 py-2 rounded-lg text-xs font-semibold bg-white/10 backdrop-blur-sm border border-white/20 text-white">
                    {yazlikListings.length}+ Aktif Yazlık İlanı
                  </span>
                </div>
          ...
          <div className="container mx-auto px-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
              <div className="text-center">
                <div className="text-2xl font-bold text-primary">{yazlikListings.length}</div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-12">
                ...
                <ScrollReveal direction="up" delay={100}>
                  <article>
                    <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
                      Satılık Yazlık Arayanlar İçin Genel Bakış
                    </h2>
                ...
                <ScrollReveal direction="up" delay={200}>
                  <div>
                    <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
                      Oda Sayısına Göre Satılık Yazlık Seçenekleri
                    </h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {/* 1+1 */}
                      <div className="p-6 bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl border border-blue-200">
                        <div className="flex items-center justify-between mb-4">
                          <h3 className="text-xl font-bold text-gray-900">1+1 Yazlık</h3>
                        ...
                        <Link href={`${basePath}/satilik?propertyType=yazlik&rooms=1`}>
                      ...
                      <div className="p-6 bg-gradient-to-br from-green-50 to-green-100 rounded-xl border border-green-200">
                        <div className="flex items-center justify-between mb-4">
                          <h3 className="text-xl font-bold text-gray-900">2+1 Yazlık</h3>
                        ...
                        <Link href={`${basePath}/satilik?propertyType=yazlik&rooms=2`}>
                      ...
                      <div className="p-6 bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl border border-purple-200">
                        <div className="flex items-center justify-between mb-4">
                          <h3 className="text-xl font-bold text-gray-900">3+1 Yazlık</h3>
                        ...
                        <Link href={`${basePath}/satilik?propertyType=yazlik&rooms=3`}>
                      ...
                      <div className="p-6 bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl border border-amber-200">
                        <div className="flex items-center justify-between mb-4">
                          <h3 className="text-xl font-bold text-gray-900">4+1 Yazlık</h3>
                        ...
                        <Link href={`${basePath}/satilik?propertyType=yazlik&rooms=4`}>
                ...
                {/* Featured Listings */}
                {yazlikListings.length > 0 && (
                  <ScrollReveal direction="up" delay={300}>
                    <div>
                      <div className="flex items-center justify-between mb-6">
                        <h2 className="text-3xl md:text-4xl font-bold text-gray-900">
                          Öne Çıkan Satılık Yazlık İlanları
                        </h2>
                        <Link href={`${basePath}/satilik?propertyType=yazlik`}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {yazlikListings.slice(0, 6).map((listing) => (
                ...
                <ScrollReveal direction="up" delay={400}>
                  <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-8 border border-gray-200">
                    <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
                      Satılık Yazlık Fiyat Analizi ve Piyasa Trendleri
                    </h2>
                    <div className="prose prose-lg max-w-none text-gray-700 space-y-4">
                      <p>
                ...
                <ScrollReveal direction="up" delay={500}>
                  <div>
                    <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
                      Yazlık Alırken Dikkat Edilmesi Gerekenler
                    </h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
                        ...
                        <p className="text-gray-700 text-sm leading-relaxed">
                          Yazlık fiyatı, noter masrafları, tapu harcı, aidat ve diğer giderler toplam maliyeti oluşturur.
                        </p>
                      </div>
              ...
              <aside className="space-y-6">
                ...
                <div className="p-6 bg-gradient-to-br from-primary/10 to-primary/5 rounded-xl border border-primary/20">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-gray-900">{yazlikListings.length}</span>
                    </div>
                    <div className="flex justify-between">
                ...
                <div className="p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
                  ...
                  <div className="space-y-2">
                    <Link href={`${basePath}/karasu-satilik-yazlik`} className="block p-3 rounded-lg hover:bg-gray-50 transition-colors">
                      <div className="font-semibold text-gray-900">Karasu Satılık Yazlık</div>
          ...
          <div className="container mx-auto px-4 max-w-4xl text-center">
            <ScrollReveal direction="up" delay={0}>
              <h2 className="text-3xl md:text-4xl font-bold mb-4">
                Hayalinizdeki Yazlıkyi Bulun
              </h2>
              <p className="text-lg text-white/90 mb-8 max-w-2xl mx-auto">
                Geniş ilan yelpazesi ve uzman danışmanlık hizmeti ile size en uygun yazlığı bulmanıza yardımcı oluyoruz.
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
