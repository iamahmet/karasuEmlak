...
const RealTimeStats = dynamic(
  () => import("@/components/admin/dashboard/RealTimeStats").then((mod) => ({ default: mod.RealTimeStats })),
  { loading: () => <PageSkeleton />, ssr: true }
);

const ActivityFeed = dynamic(
  () => import("@/components/admin/activity-feed/ActivityFeed").then((mod) => ({ default: mod.ActivityFeed })),
  { loading: () => <PageSkeleton /> }
);

const QuickStats = dynamic(
  () => import("@/components/admin/dashboard/QuickStats").then((mod) => ({ default: mod.QuickStats })),
  { loading: () => <PageSkeleton /> }
);

const RecentActivity = dynamic(
  () => import("@/components/admin/dashboard/RecentActivity").then((mod) => ({ default: mod.RecentActivity })),
  { loading: () => <PageSkeleton /> }
);

const TopContent = dynamic(
  () => import("@/components/admin/dashboard/TopContent").then((mod) => ({ default: mod.TopContent })),
  { loading: () => <PageSkeleton /> }
);

const PerformanceMetrics = dynamic(
  () => import("@/components/admin/dashboard/PerformanceMetrics").then((mod) => ({ default: mod.PerformanceMetrics })),
  { loading: () => <PageSkeleton /> }
);
...
const QuickActions = dynamic(
  () => import("@/components/admin/dashboard/QuickActions").then((mod) => ({ default: mod.QuickActions })),
  { loading: () => <PageSkeleton /> }
);

const TrendingContent = dynamic(
  () => import("@/components/admin/dashboard/TrendingContent").then((mod) => ({ default: mod.TrendingContent })),
  { loading: () => <PageSkeleton /> }
);

const ContentCalendar = dynamic(
  () => import("@/components/admin/dashboard/ContentCalendar").then((mod) => ({ default: mod.ContentCalendar })),
  { loading: () => <PageSkeleton /> }
);

const QuickInsights = dynamic(
  () => import("@/components/admin/dashboard/QuickInsights").then((mod) => ({ default: mod.QuickInsights })),
  { loading: () => <PageSkeleton /> }
);

const StatsChart = dynamic(
  () => import("@/components/admin/dashboard/StatsChart").then((mod) => ({ default: mod.StatsChart })),
  { loading: () => <PageSkeleton /> }
);

const RealTimeUpdates = dynamic(
  () => import("@/components/admin/dashboard/RealTimeUpdates").then((mod) => ({ default: mod.RealTimeUpdates })),
  { loading: () => <PageSkeleton /> }
);
...
export default async function DashboardPage({
  ...
  let t: any = (key: string) => key; // Default fallback
  try {
    t = await getTranslations({ locale, namespace: "admin.dashboard" });
  } catch (error) {
    console.error('Error loading translations:', error);
    // Fallback to empty translations - already set above
  }
  ...
  try {
    ...
    const [articlesResult, categoriesResult, usersResult, viewsResult] = await Promise.allSettled([
    ...
    totalCategories = categoriesResult.status === 'fulfilled' ? (categoriesResult.value.count || 0) : 0;
    totalUsers = usersResult.status === 'fulfilled' ? (usersResult.value.count || 0) : 0;
    totalViews = viewsResult.status === 'fulfilled' 
      ? (viewsResult.value.data?.reduce((sum: number, a: any) => sum + (Number(a.views) || 0), 0) || 0)
      : 0;

    // Recent articles are handled by TopContent component

    // Get published articles count with error handling
    try {
