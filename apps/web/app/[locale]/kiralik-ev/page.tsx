...
export async function generateMetadata({
  ...
  const canonicalPath = locale === routing.defaultLocale ? '/kiralik-ev' : `/${locale}/kiralik-ev`;
  
  const baseDescription = 'Türkiye\'de kiralık ev ilanları. 1+1\'den 4+1\'e kadar seçenek. Güncel fiyatlar, mahalle rehberi ve yatırım analizi. Uzman emlak danışmanlığı ile hayalinizdeki evi bulun.';
  const optimizedDescription = optimizeMetaDescription(baseDescription, {
    keywords: ['kiralık ev', 'kiralık ev ilanları', 'emlak'],
    includeCTA: true,
  });

  return {
    title: 'Kiralık Ev | En Güncel İlanlar ve Fiyatlar 2025 | Karasu Emlak',
    ...
    openGraph: {
      ...
      description: 'Türkiye\'de satılık ev ilanları. 1+1\'den 4+1\'e kadar geniş seçenek. Güncel fiyatlar ve mahalle rehberi.',
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: 'Kiralık Ev - Emlak İlanları ve Fiyatları',
        },
      ],
    ...
    twitter: {
      ...
      title: 'Kiralık Ev | En Güncel İlanlar',
      description: 'Türkiye\'de satılık ev ilanları. Geniş seçenek. Güncel fiyatlar ve mahalle rehberi.',
    },
    robots: {
      index: true,
      follow: true,
...
async function getKiralikEvFAQs() {
  ...
  if (allFAQs.length === 0) {
    allFAQs.push(
      ...
      {
        question: 'Ev alırken hangi belgeler gereklidir?',
        answer: 'Ev alırken tapu, yapı ruhsatı, iskan belgesi, vekaletname (varsa), kimlik belgeleri, gelir belgesi (kredi için), vergi levhası ve diğer yasal belgeler gereklidir. Tüm belgelerin güncel ve eksiksiz olması önemlidir.',
      },
      {
      ...
      {
        question: 'Ev alırken ekspertiz yaptırmak gerekli mi?',
...
export default async function KiralikEvPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const [allListingsResult, neighborhoodsResult, statsResult] = await Promise.all([
    withTimeout(
      getListings({ status: 'kiralik', property_type: ['ev'] }, { field: 'created_at', order: 'desc' }, 1000, 0),
      3000,
      { listings: [], total: 0 }
    ),
  ...
  const evListings = allListings.filter(listing => listing.property_type === 'daire');

  // Group by room count
  const byRooms = {
    '1+1': evListings.filter(l => l.features?.rooms === 1),
    '2+1': evListings.filter(l => l.features?.rooms === 2),
    '3+1': evListings.filter(l => l.features?.rooms === 3),
    '4+1': evListings.filter(l => l.features?.rooms === 4),
  };

  // Calculate average price
  const prices = evListings
  ...
  const faqsResult = await withTimeout(getKiralikEvFAQs(), 2000, []);
  const faqs = faqsResult || [];

  // Generate schemas
  const articleSchema = {
    ...generateArticleSchema({
      headline: 'Kiralık Ev | En Güncel İlanlar ve Fiyatlar 2025',
      description: 'Türkiye\'de satılık ev ilanları. 1+1\'den 4+1\'e kadar geniş seçenek. Güncel fiyatlar, mahalle rehberi ve yatırım analizi.',
  ...
  const breadcrumbSchema = generateBreadcrumbSchema(
    [
      ...
      { name: 'Kiralık Ev', url: `${siteConfig.url}${basePath}/kiralik-ev` },
    ],
    `${siteConfig.url}${basePath}/kiralik-ev`
  );
  ...
  const itemListSchema = evListings.length > 0
    ? generateItemListSchema(evListings.slice(0, 20), `${siteConfig.url}${basePath}`, {
        name: 'Kiralık Ev İlanları',
        description: `Türkiye'de ${evListings.length} adet satılık daire ilanı. Geniş seçenek.`,
      })
    : null;
  // Generate page content for AI checker
  const pageContentInfo = generatePageContentInfo('Kiralık Ev', [
    { 
      id: 'genel-bakis', 
      title: 'Kiralık Ev Arayanlar İçin Genel Bakış', 
      content: 'Türkiye\'de kiralık ev ilanları ve seçenekleri hakkında kapsamlı bilgi. Farklı bölgelerde, farklı fiyat aralıklarında ve farklı özelliklerde ev seçenekleri bulunmaktadır. Hem sürekli oturum hem de yatırım amaçlı seçenekler mevcuttur.' 
    },
    { 
      id: 'oda-sayisina-gore', 
      title: 'Oda Sayısına Göre Kiralık Ev Seçenekleri', 
      content: '1+1, 2+1, 3+1 ve 4+1 oda seçenekleri mevcuttur. Tek kişi veya çiftler için 1+1, küçük aileler için 2+1, orta büyüklükte aileler için 3+1, büyük aileler için 4+1 ev seçenekleri bulunmaktadır.' 
    },
    { 
      id: 'fiyat-analizi', 
      title: 'Kiralık Ev Fiyat Analizi', 
      content: 'Kiralık ev fiyatları bölge, konum, metrekare, oda sayısı ve özelliklere göre değişmektedir. Ortalama aylık kira fiyatları 1+1 evler için 3.000 TL - 8.000 TL, 2+1 evler için 5.000 TL - 12.000 TL, 3+1 evler için 7.000 TL - 15.000 TL, 4+1 evler için 10.000 TL - 20.000 TL arasında değişmektedir.' 
    },
    { 
      id: 'mahalle-rehberi', 
      title: 'Mahalle Rehberi', 
      content: 'Popüler mahalleler ve özellikleri. Merkez mahalleler, gelişen bölgeler, sahil bölgeleri ve uygun fiyatlı bölgeler hakkında detaylı bilgi. Her mahallenin ulaşım, sosyal alanlar, okullar ve sağlık kuruluşları açısından değerlendirmesi.' 
    },
    { 
      id: 'yatirim-tavsiyeleri', 
      title: 'Yatırım Tavsiyeleri', 
      content: 'Kiralık ev yatırımı yapmayı düşünenler için kira getirisi, bölgenin gelişim potansiyeli, gelecek projeleri ve piyasa trendleri hakkında tavsiyeler. Yatırım amaçlı ev alımında dikkat edilmesi gerekenler ve getiri analizi.' 
    },
  ]);
    ...
    <>
      ...
      <Breadcrumbs
        items={[
          ...
          { label: 'Kiralık Ev', href: `${basePath}/kiralik-ev` },
        ]}
      />
              ...
              <div className="max-w-4xl mx-auto text-center">
                <div className="inline-block mb-4">
                  <span className="px-4 py-2 rounded-lg text-xs font-semibold bg-white/10 backdrop-blur-sm border border-white/20 text-white">
                    {evListings.length}+ Aktif Ev İlanı
                  </span>
                </div>
                ...
                <p className="text-lg md:text-xl text-gray-200 max-w-3xl mx-auto mb-8">
                  ...
                  Güncel fiyatlar, mahalle rehberi ve yatırım analizi. Uzman emlak danışmanlığı ile hayalinizdeki evi bulun.
                </p>
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  <Button asChild size="lg" className="bg-white text-gray-900 hover:bg-gray-100">
                    <Link href={`${basePath}/kiralik?propertyType=ev`}>
                      <Home className="w-5 h-5 mr-2" />
                      Tüm Ev İlanlarını Görüntüle
                    </Link>
                  </Button>
          ...
          <div className="container mx-auto px-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
              <div className="text-center">
                <div className="text-2xl font-bold text-primary">{evListings.length}</div>
                <div className="text-sm text-gray-600">Aktif Ev İlanı</div>
              </div>
              <div className="text-center">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-12">
                ...
                <ScrollReveal direction="up" delay={100}>
                  <article>
                    <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
                      Kiralık Ev Arayanlar İçin Genel Bakış
                    </h2>
                ...
                <ScrollReveal direction="up" delay={200}>
                  <div>
                    <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
                      Oda Sayısına Göre Kiralık Ev Seçenekleri
                    </h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {/* 1+1 */}
                      <div className="p-6 bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl border border-blue-200">
                        <div className="flex items-center justify-between mb-4">
                          <h3 className="text-xl font-bold text-gray-900">1+1 Ev</h3>
                        ...
                        <Link href={`${basePath}/kiralik?propertyType=ev&rooms=1`}>
                      ...
                      <div className="p-6 bg-gradient-to-br from-green-50 to-green-100 rounded-xl border border-green-200">
                        <div className="flex items-center justify-between mb-4">
                          <h3 className="text-xl font-bold text-gray-900">2+1 Ev</h3>
                        ...
                        <Link href={`${basePath}/kiralik?propertyType=ev&rooms=2`}>
                      ...
                      <div className="p-6 bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl border border-purple-200">
                        <div className="flex items-center justify-between mb-4">
                          <h3 className="text-xl font-bold text-gray-900">3+1 Ev</h3>
                        ...
                        <Link href={`${basePath}/kiralik?propertyType=ev&rooms=3`}>
                      ...
                      <div className="p-6 bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl border border-amber-200">
                        <div className="flex items-center justify-between mb-4">
                          <h3 className="text-xl font-bold text-gray-900">4+1 Ev</h3>
                        ...
                        <Link href={`${basePath}/kiralik?propertyType=ev&rooms=4`}>
                ...
                {evListings.length > 0 && (
                  <ScrollReveal direction="up" delay={300}>
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {evListings.slice(0, 6).map((listing) => (
                ...
                <ScrollReveal direction="up" delay={400}>
                  <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-8 border border-gray-200">
                    <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
                      Kiralık Ev Fiyat Analizi ve Piyasa Trendleri
                    </h2>
                    <div className="prose prose-lg max-w-none text-gray-700 space-y-4">
                      <p>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
                        ...
                        <p className="text-gray-700 text-sm leading-relaxed">
                          Ev fiyatı, noter masrafları, tapu harcı, aidat ve diğer giderler toplam maliyeti oluşturur.
                        </p>
                      </div>
              ...
              <aside className="space-y-6">
                ...
                <div className="p-6 bg-gradient-to-br from-primary/10 to-primary/5 rounded-xl border border-primary/20">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-gray-900">{evListings.length}</span>
                    </div>
                    <div className="flex justify-between">
                ...
                <div className="p-6 bg-white rounded-xl border border-gray-200 shadow-sm">
                  ...
                  <div className="space-y-2">
                    <Link href={`${basePath}/karasu-kiralik-ev`} className="block p-3 rounded-lg hover:bg-gray-50 transition-colors">
                      <div className="font-semibold text-gray-900">Karasu Kiralık Ev</div>
                    ...
                    <Link href={`${basePath}/kiralik-villa`} className="block p-3 rounded-lg hover:bg-gray-50 transition-colors">
                      <div className="font-semibold text-gray-900">Kiralık Villa</div>
                      <div className="text-sm text-gray-600">Kiralık villa ilanları</div>
                    </Link>
                    <Link href={`${basePath}/satilik-ev`} className="block p-3 rounded-lg hover:bg-gray-50 transition-colors">
                      ...
                      <div className="text-sm text-gray-600">Satılık ev ilanları</div>
                    </Link>
          ...
          <div className="container mx-auto px-4 max-w-4xl text-center">
            <ScrollReveal direction="up" delay={0}>
              <h2 className="text-3xl md:text-4xl font-bold mb-4">
                Hayalinizdeki Evyi Bulun
              </h2>
              <p className="text-lg text-white/90 mb-8 max-w-2xl mx-auto">
                Geniş ilan yelpazesi ve uzman danışmanlık hizmeti ile size en uygun evi bulmanıza yardımcı oluyoruz.
              </p>
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
