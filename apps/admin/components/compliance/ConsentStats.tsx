    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
      {stats.map((stat, index) => {
        ...
        return (
          <Card
            key={stat.label}
            className="card-professional bg-white dark:bg-card hover-lift animate-slide-up group cursor-pointer relative overflow-hidden"
          ...
          >
            ...
            <CardHeader className="flex flex-row items-center justify-between pb-3 px-5 pt-5 relative z-10">
              <CardTitle className="text-[10px] md:text-xs font-ui font-bold text-muted-foreground uppercase tracking-wider">
                {stat.label}
              </CardTitle>
            ...
            <CardContent className="px-5 pb-5 relative z-10">
              <div className="flex items-baseline justify-between mb-4">
                <div className="text-3xl md:text-4xl font-display font-bold text-foreground tracking-tight bg-gradient-to-br from-design-dark to-design-dark/80 dark:from-white dark:to-gray-300 bg-clip-text text-transparent">
                  {stat.value}
                </div>
              </div>
              <p className="text-[10px] md:text-xs text-muted-foreground font-ui font-medium">
                {stat.description}
              </p>
            </CardContent>
          </Card>
