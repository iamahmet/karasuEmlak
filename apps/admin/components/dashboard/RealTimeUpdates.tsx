    ...
    const articlesChannel = supabase
      ...
      .on(
        ...
        ((payload: any) => {
          ...
          setUpdates((prev) => [
            {
              id: `article-${newRecord?.id || oldRecord?.id}-${Date.now()}`,
              type: "article",
              action,
              title: newRecord?.title || oldRecord?.title || "Makale",
              timestamp: new Date(),
            },
    ...
    const listingsChannel = supabase
      ...
      .on(
        ...
        ((payload: any) => {
          ...
          setUpdates((prev) => [
            {
              id: `listing-${newRecord?.id || oldRecord?.id}-${Date.now()}`,
              type: "listing",
              action,
              title: newRecord?.title || oldRecord?.title || "İlan",
              timestamp: new Date(),
            },
            ...prev.slice(0, 9),
          ]);
