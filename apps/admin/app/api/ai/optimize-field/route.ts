...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (field) {
      case "title":
        prompt = TITLE_OPTIMIZATION_PROMPT(currentValue, context.content || "");
        break;

      case "meta_description":
        prompt = META_DESCRIPTION_PROMPT(context.title || "", context.content || "", currentValue);
        break;

      case "seo_keywords":
        prompt = SEO_KEYWORDS_PROMPT(context.title || "", context.content || "", currentValue);
        break;

      case "excerpt":
        prompt = EXCERPT_OPTIMIZATION_PROMPT(context.content || "", currentValue);
        break;

      case "content":
        prompt = CONTENT_OPTIMIZATION_PROMPT(currentValue || "", "blog");
        break;
