...
/**
 * Generate images for image suggestions in news content
 */
export async function POST(
  ...
  try {
    ...
    const { field = 'emlak_analysis' } = body;
    ...
    console.log(`[News Generate Images] Found ${suggestions.length} image suggestions`);
    ...
    for (const suggestion of suggestions) {
      try {
        console.log(`[News Generate Images] Generating image for: ${suggestion.imageIdea}`);
        ...
        const imageResponse = await fetch(`${baseUrl}/api/ai/generate-image`, {
          ...
          body: JSON.stringify({
            ...
            upload: {
              ...
              tags: ['ai-generated', 'news', 'auto-generated'],
            },
          }),
        });
        ...
        if (imageData.success && imageData.url) {
          ...
          updatedContent = updatedContent.replace(
            ...
            `<img src="${imageData.url}" alt="${suggestion.altText}" class="w-full rounded-lg my-6" />`
          );
          ...
          console.log(`[News Generate Images] Successfully generated image: ${imageData.url}`);
        } else {
          console.error(`[News Generate Images] Failed to generate image:`, imageData.error);
          generatedImages.push({
            suggestion,
        ...
        console.error(`[News Generate Images] Error generating image:`, error);
        generatedImages.push({
          suggestion,
    ...
    if (generatedImages.some(img => img.imageUrl)) {
      ...
      if (updateError) {
        console.error('[News Generate Images] Error updating article:', updateError);
        return NextResponse.json(
    ...
    console.error('[News Generate Images] Error:', error);
    return NextResponse.json(
