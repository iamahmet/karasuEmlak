...
function generateSQL() {
  ...
  sqlLines.push('-- Q&A Entries for Content Completion');
  sqlLines.push('-- Generated: ' + new Date().toISOString());
  sqlLines.push('-- Total: ' + allQAs.length + ' entries (21 Karasu + 21 Kocaali)');
  sqlLines.push('');
  sqlLines.push('INSERT INTO public.qa_entries (question, answer, category, region, priority)');
  sqlLines.push('VALUES');

  const values = allQAs.map((qa, index) => {
    const comma = index < allQAs.length - 1 ? ',' : ';';
    return `  ('${escapeSQL(qa.question)}', '${escapeSQL(qa.answer)}', '${qa.category}', '${qa.region}', '${qa.priority}')${comma}`;
  });

  sqlLines.push(...values);
  sqlLines.push('');
  sqlLines.push('-- Note: This uses ON CONFLICT DO NOTHING if unique constraint exists');
  sqlLines.push('-- If you want to update existing entries, use:');
  sqlLines.push('-- ON CONFLICT (question, region) DO UPDATE SET answer = EXCLUDED.answer, category = EXCLUDED.category, priority = EXCLUDED.priority;');
...
async function main() {
  ...
  const outputPath = path.join(process.cwd(), 'scripts/content/qa-entries-insert.sql');
  
  fs.writeFileSync(outputPath, sql);
  
  console.log('✅ SQL file generated!');
  console.log(`📄 Location: ${outputPath}`);
  console.log(`📊 Total entries: ${karasuQAs.length + kocaaliQAs.length}`);
  console.log('\n💡 Run this SQL in Supabase SQL Editor to insert Q&A entries.');
}
...
export { generateSQL, karasuQAs, kocaaliQAs };
