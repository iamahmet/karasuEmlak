...
-- Ensure expected columns exist even when authors table was created earlier with a different shape.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'authors' AND column_name = 'avatar_media_id'
  ) THEN
    ALTER TABLE public.authors ADD COLUMN avatar_media_id uuid;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'authors' AND column_name = 'cover_media_id'
  ) THEN
    ALTER TABLE public.authors ADD COLUMN cover_media_id uuid;
  END IF;
END $$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'articles') THEN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'articles' AND column_name = 'primary_author_id') THEN
      ...
      IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'authors') THEN
        ALTER TABLE public.articles DROP CONSTRAINT IF EXISTS articles_primary_author_id_fkey;
        ALTER TABLE public.articles
